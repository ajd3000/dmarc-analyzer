...
- **Dedupes** by `source_ip` per request. At most **300** unique **public** IPs are queried in parallel; extra unique IPs show **`(RDAP lookup cap)`**. Large reports may take **tens of seconds** on first analyze. RDAP registries sometimes **rate-limit** bursts; the app uses **moderate parallelism**, **per-IP retries**, and a **second sequential pass** for any IPs that still returned **"—"** after the first batch.
